<template>
    ...
    <tr>
         <th></th>
        <td><button @click="insert">seve</button>
        <transition name="del">
...
export befault{
    ...
    computed:{
        ...
        page_items:function(){
            if(this.find_flg){
                var arr=[];
                var date=this.$store.state.memo.memo;
                date.forEach(element =>{
                    if(element.title.toLowerCase())>=0{
                        arr.push(element);
                    }
                });
                return arr;
                ...
                return this.$store.state.memo.memo.slice(
                    this.unm_per_page * this.$store.state.memo.page,
                    this.unm_per_page * ( this.$store.state.memo.page + 1));
            }
        },
        page:{
            get:function(){
            ...
            set:function(p){
                var pg=p > ( this.$store.state.memo.nemo.length - 1)
                / this.unm_per_page ?
                Math.ceil(( this.$store.state.memo.page.length -1)
                /this.unm_per_page) -1 :p;
                pg= pg < 0 ? 0 :pg;
    ...
    created:function(){
        this.$store.commit('memo/set_pege',0);
    },
}
</script>
